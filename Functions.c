...
void insertValuesMatrixA()
{
    ...
    for (i = 0; i < rowsMatrixA; ++i) // ciclo principal para correr numero de linhas definidas
        for (j = 0; j < columsMatrixA; ++j) // ciclo secundario para correr o numero de colunas definidas. Este ciclo for está dentro do outro para ele correr todas as colunas da linha i, só qdo acabar as colunas é que vai para a linha seguinte
        {
...
void insertValuesMatrixB()
{
    ...
    for (i = 0; i < rowsMatrixB; ++i)  // ciclo principal para correr numero de linhas definidas
        for (j = 0; j < columsMatrixB; ++j) // ciclo secundario para correr o numero de colunas definidas. Este ciclo for está dentro do outro para ele correr todas as colunas da linha i, só qdo acabar as colunas é que vai para a linha seguinte
        {
...
int SumMatrix(int matrixAA[10][10], int  matrixBB[10][10])  //inicia função para somar 2 matrizes com até 10 linhas e 10 colunas
{
    ...
    */
    ...
    for (i = 0; i < rowsMatrixA; ++i)  //cico para criar as linhas pra as nossas matrizes 
        for (j = 0; j < columsMatrixA; ++j) //cico para criar as colunas pra as nossas matrizes 
        {
            matrixResult[i][j] = matrixAA[i][j] + matrixBB[i][j]; //guarda no array o valor das somas das duas matrizes
        }

    for (i = 0; i < rowsMatrixA; ++i) //ciclo para apresentar as linhas 
        for (j = 0; j < columsMatrixA; ++j)  //ciclo para apresentar as colunas 
        {
...
int MultiplyMatrix(int matrixAA[10][10], int  matrixBB[10][10])  //inicia função para multiplicar 2 matrizes com até 10 linhas e 10 colunas
{
    /*Regras para subtrair 2 matrizes:
  * - As 2 matrizes têm obrigatoriamente de ser iguais, nº de linhas e colunas
  * - A subtração é feita entre os elementos das 2 matrizes, por exemplo:
  ...
  */
    ...
    for (i = 0; i < rowsMatrixA; ++i)  //cico para criar as linhas pra as nossas matrizes 
        for (j = 0; j < columsMatrixA; ++j) //cico para criar as colunas pra as nossas matrizes 
        {
            matrixResult[i][j] = matrixAA[i][j] * matrixBB[j][i]; //guarda no array resultado o valor da subtração das duas matrizes
        }
